...
export class SocketGateway implements OnGatewayInit {
  ...
  private messagesList: MassagePayload[] = []
  ...
  public async handleMessage(client: WebSocket, payload: string): Promise<void> {
    ...
    const newMessage: MassagePayload = { event: 'message', text: payload, memberData: authMember };
    const clientNick: string = authMember?.memberNick ?? 'Guest';
  ...
  private broadcastMessage(sender: WebSocket, message: InfoPayload | MassagePayload) {
  ...
  private emitMessage(message: InfoPayload | MassagePayload) {
    this.server.clients.forEach((client) => {
